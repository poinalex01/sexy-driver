...
class C4 {
   public:
    Vector3 GetPlantedC4Position() {
        uintptr_t Planted_C4_Node =
            Memory::ReadMemory<uintptr_t>(game.Planted_C4 + Offsets::m_pGameSceneNode);
        return Memory::ReadMemory<Vector3>(Planted_C4_Node +
                                           Offsets::m_vecAbsOrigin);  // C4 Position
    }

    Vector3 GetDroppedC4Position() {
        uintptr_t Dropped_C4_Node =
            Memory::ReadMemory<uintptr_t>(game.Weapon_C4 + Offsets::m_pGameSceneNode);
        return Memory::ReadMemory<Vector3>(Dropped_C4_Node +
                                           Offsets::m_vecAbsOrigin);  // C4 Position
    }
