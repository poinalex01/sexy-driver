...
namespace DrawUtils {
    inline void DrawBox(const Vector2& topLeft, const Vector2& bottomRight,
                        ImColor color = ImColor(255, 255, 255, 255), float thickness = 2.0f) {
        ImGui::GetBackgroundDrawList()->AddRect(ImVec2(topLeft.x, topLeft.y),
                                                ImVec2(bottomRight.x, bottomRight.y), color, 0.0f,
                                                ImDrawFlags_None, thickness);
    }

    inline void DrawLine(const Vector2& from, const Vector2& to,
                         ImColor color = ImColor(255, 255, 255, 255), float thickness = 1.0f,
                         bool drawEndPoint = false, float endPointRadius = 3.0f,
                         ImColor endPointColor = ImColor(255, 255, 255, 255)) {
        ImGui::GetBackgroundDrawList()->AddLine(ImVec2(from.x, from.y), ImVec2(to.x, to.y), color,
                                                thickness);

        if (drawEndPoint) {
            ImGui::GetBackgroundDrawList()->AddCircleFilled(ImVec2(to.x, to.y), endPointRadius,
                                                            endPointColor);
        }
    }

    inline void DrawHealthBar(const Vector2& topLeft, const Vector2& bottomRight, int health,
                              ImColor fillColor = ImColor(0, 255, 0, 255), float barWidth = 7.0f,
                              float offset = 5.0f) {
        ...
        ImVec2 healthTopLeft(topLeft.x - barWidth - offset,
                             bottomRight.y - healthHeight);  // Adjust based on health
        ImVec2 healthBottomRight(topLeft.x - offset, bottomRight.y);
        ImGui::GetBackgroundDrawList()->AddRectFilled(healthTopLeft, healthBottomRight, fillColor,
                                                      0.0f, ImDrawFlags_None);
    }

    inline void DrawTextW(const char* text, const Vector2& position, ImU32 color,
                          bool centered = false) {
        ImGuiIO& io = ImGui::GetIO();
        ImFont* font = ImGui::GetFont();  // Get the current font
        ImVec2 textSize = font->CalcTextSizeA(io.FontGlobalScale, FLT_MAX, 0.0f,
                                              text);  // Get the size of the text

        // Apply global font scale if needed
        float globalScale = io.FontGlobalScale;

        if (centered) {
            ImVec2 textPos(position.x - textSize.x * globalScale / 2.0f,
                           position.y - textSize.y * globalScale / 2.0f);
    ...
    inline void DrawCircleFilled(float x, float y, float radius,
            ...
            if (WorldToScreen(entities->Position, screenPos, game.ViewMatrix) &&
                ...
                if (WorldToScreen(VectorBone1, b1, game.ViewMatrix) &&
                    ...
                    if (!Config::Esp::ShowBoneJoints) {
                        DrawUtils::DrawLine(b1, b2, Config::Esp::BonesColor,
                                            Config::Esp::BonesThickness);
                    } else {
                        if (game.Distance_From_LocalPlayer_To_Entities <
                            Config::Esp::JointDisplayDistance) {
                            DrawUtils::DrawLine(b1, b2, Config::Esp::BonesColor,
                                                Config::Esp::BonesThickness, true, 2.0f,
                    ...
                    if (!Config::Esp::HeadFilled) {
                        DrawUtils::DrawCircle(headScreenPos.x, headScreenPos.y, headHeight,
                                              Config::Esp::HeadColor, Config::Esp::BonesThickness);
                    } else {
                        DrawUtils::DrawCircleFilled(headScreenPos.x, headScreenPos.y, headHeight,
                                                    Config::Esp::HeadColor);
                    }
                }
            }
        }
    ...
    inline void DrawCrosshair(float crosshairSize = 10.0f,
                              ImColor color = ImColor(255, 255, 255, 255)) {
    ...
    inline void DrawAimFovCircle() {
        ...
        draw_list->AddCircle(
            ...
            Config::Aimbot::AimFov, ImGui::ColorConvertFloat4ToU32(Config::Aimbot::FovCircleColor),
            100, Config::Aimbot::FovCircleThickness);
    }

    inline void Draw3DBox(Vector3* corners, ImColor color = ImColor(255, 255, 255, 255),
