...
Vector2 AIMBOT::FindClosestEnemy() {
    ...
    for (int i = 1; i < 64; i++) {
        ...
        if (Config::Aimbot::TeamCheck &&
            entities->Team == player.Team)  // Avoiding Aimbot For Team;
            continue;
        ...
        switch (Config::Aimbot::CurrentAimPosIndex) {
            case 0:
                AimPosition3D = Head3D;
                break;
            case 1:
                AimPosition3D = Penis3D;
                break;
            case 2:
                AimPosition3D = Neck3D;
                break;
            case 3:
                AimPosition3D = Chest3D;
                break;
            case 4:
                AimPosition3D = Stomach3D;
                break;
            default:
                AimPosition3D = Head3D;
                break;
        ...
        if (WorldToScreen(AimPosition3D, enemyScreenPos, game.ViewMatrix)) {
            float distance = sqrt(pow(enemyScreenPos.x - Center_Of_Screen.x, 2) +
                                  pow(enemyScreenPos.y - Center_Of_Screen.y, 2));
