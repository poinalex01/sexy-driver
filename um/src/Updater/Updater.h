...
class Updater {
   private:
    ...
    string Github_Repo_Api_NeoXa7 =
        "https://api.github.com/repos/NeoXa7/Cpp-Updater-Class-for-CS2-Offsets/commits";
    ...
    const std::vector<std::pair<std::string, std::string>> Github_File_Path = {
        ...
        {"https://github.com/NeoXa7/Cpp-Updater-Class-for-CS2-Offsets/raw/main/Client_Dll.json",
         "Client_Dll.json"},
   ...
   public:
    inline bool CheckAndDownload() {
        ...
        if (!GetLastCommitInfo(Github_Repo_Api_A2X, A2x_Commit)) {
            std::cout << " [Updater] Error getting last commit information from GitHub"
                      << std::endl;
            return false;
        }
        ...
        for (const auto& file : Github_File_Path) {
            ...
            auto lastModifiedTime =
                fileExists ? fileSys::last_write_time(localPath) : fileSys::file_time_type{};

            auto lastModifiedClockTime =
                fileExists ? std::chrono::time_point_cast<std::chrono::system_clock::duration>(
                                 lastModifiedTime - fileSys::file_time_type::clock::now() +
                                 std::chrono::system_clock::now())
                           : std::chrono::system_clock::time_point{};

            if (fileExists) {
                if (lastModifiedClockTime < CommitTimePoint_A2X ||
                    lastModifiedClockTime < CommitTimePoint_NeoXa7) {
                    if (localPath == "buttons.json") {
                        if (DownloadFile(url, localPath)) {
                            std::cout << " [Updater] buttons.json : Successfully Updated to latest "
                                         "Offsets\n";
                        } else {
                            std::cout
                                << " [Updater] Failed to Update to the latest Offsets. File Name : "
                                << localPath << " Try downloading manually from " << url << '\n';
                        }
                    }

                    if (localPath == "offsets.json") {
                        if (DownloadFile(url, localPath)) {
                            std::cout << " [Updater] offsets.json : Successfully Updated to latest "
                                         "Offsets\n";
                        } else {
                            std::cout
                                << " [Updater] Failed to Update to the latest Offsets. File Name : "
                    ...
                    if (localPath == "Client_Dll.json") {
                        if (DownloadFile(url, localPath)) {
                            std::cout << " [Updater] Client_Dll.json : Successfully Updated to "
                                         "latest Offsets\n";
                        } else {
                            std::cout
                                << " [Updater] Failed to Update to the latest Offsets. File Name : "
                ...
                if (DownloadFile(url, localPath)) {
                    std::cout << " [Updater] Successfully downloaded the latest " << localPath
                              << "." << std::endl;
                } else {
                    std::cout << " [Updater] Error: Failed to download " << localPath
                              << ". Try downloading manually from " << url << std::endl;
                }
            } else {
    ...
    inline bool UpdateOffsets() {
        ...
        for (const auto& FileNames : FileName) {
            ...
            try {
                ...
                std::cout << " [Updater] Failed to parse JSON from " << FileNames << ": "
                          << e.what() << std::endl;
                return false;
            }
