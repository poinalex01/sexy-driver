   ...
   public:
    void UpdateGameVars() {
        ...
        Planted_C4 = Memory::ReadMemory<uintptr_t>(
            Planted_C4 = Memory::ReadMemory<uintptr_t>(
                CS2::ClientDll + Offsets::dwPlantedC4));  // Derefrencing Memory;

        Weapon_C4 = Memory::ReadMemory<uintptr_t>(
            Weapon_C4 = Memory::ReadMemory<uintptr_t>(
                CS2::ClientDll + Offsets::dwWeaponC4));  // Derefrencing Memory;

        CameraServices =
            Memory::ReadMemory<uintptr_t>(player.localPlayerPawn + Offsets::m_pCameraServices);

        uintptr_t MapNameAddress = Memory::ReadMemory<uintptr_t>(
            CS2::MatchMakingDll + Offsets::dwGameTypes + Offsets::dwGameTypes_mapName + 8);
        ...
        FlashDuration =
            Memory::ReadMemory<float>(player.localPlayerPawn + Offsets::m_flFlashBangTime);
        ...
        Distance_From_LocalPlayer_To_Entities =
            (entities->Position - player.Position).Magnitude();  // In Units;
        IsTerroristTimeOutActive =
            Memory::ReadMemory<bool>(Rules + Offsets::m_bTerroristTimeOutActive);
