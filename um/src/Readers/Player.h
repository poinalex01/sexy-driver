...
class LocalPlayer {
   ...
   public:
    void UpdateLocalPlayer() {
        localPlayerPawn =
            Memory::ReadMemory<uintptr_t>(CS2::ClientDll + Offsets::dwLocalPlayerPawn);
        LocalPlayerController =
            Memory::ReadMemory<uintptr_t>(CS2::ClientDll + Offsets::dwLocalPlayerController);

        DW_Sensitivity =
            Memory::ReadMemory<uintptr_t>(CS2::ClientDll + Offsets::dwSensitivity);  // private
        NameAddress = Memory::ReadMemory<uintptr_t>(LocalPlayerController +
                                                    Offsets::m_sSanitizedPlayerName);  // private
        ...
        Sensitivity =
            Memory::ReadMemory<float>(DW_Sensitivity + Offsets::dwSensitivity_sensitivity);
